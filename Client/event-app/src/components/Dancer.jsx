...
const Dancer = () => {
  ...
  const getEvent = () => {
    axios.get('http://localhost:8080/discover/dancer').then((res) => {
      setEvent(res.data);
      setLoading(false);
    });
  };
  ...
  return (
    <>
    <h3 className=' font-semibold'>Dancer Artist</h3>
      {loading ? (
        <p>loading ...</p>
      ) : (
        <>
              ...
              <div className='grid grid-cols-4 gap-8'>
                {event.map((item) => (
                  <div className='flex flex-col gap-1 mt-3 ' key={item._id}>
                    ...
                    />
                    <h3 className='text-lg font-semibold'>{item.dancerName}</h3>
